...
除了首页那一步（它已经钉了四枚带说明的圆点），**每一步都能指着问**：装好的报大件名，
还没装的报它自己的名，指到底座就报车架。
...
反过来也克制 —— 看不出来的位移直接落位不动画，中段的外扩只留给真要绕过半个车身的那几趟
（实测二十八趟里只有六趟有），界面自己引起的让位走快档，且不许把正在走的那一趟掐短。
开箱那三步索性共用一个机位：装好的、摊开的、只剩车架的，镜头不动，变的只有车。
哪一章站哪一侧也是挑过的 —— 传动三步都在传动侧，左右脚踏各站自己那一侧的前四分之三位。
往回翻同样顺。

**不投影。** 一台自行车投在地上的影子是一大片辐条、链条、叉腿交织的噪点，
面积常常比主体本身还大，近景里它就摊在主体旁边把眼睛勾过去。
去掉之后主体浮在一块干净的台面上，边缘一清二楚。

**近景要看得出这是车上的哪儿，而且这一步要看的那件东西就在舞台中央。**
每一步的机位与取景由几何现算，框住这一件从预备位到装配位扫过的整段行程，
连同它周围那一小块车身；对准的是它**进场那一刻**停的地方 —— 也就是你第一眼看的地方。
主转点轴只有一颗螺栓那么大，镜头也不会贴到脸上去 —— 那样屏幕上只剩一片碳纤维编织纹。
偏心由 `npm run frames` 逐步量着：二十六步全部落在可用画面半幅的 0.18 以内，
只有车把那一步是明写的例外（它要停在中线略左）。
...
src/
  ...
  interact/slide 一自由度推入 · screw 旋入 · pick 指到哪件问哪件
...
| | 地址 | 怎么来的 |
|---|---|---|
| 主 | <https://build-bike.vercel.app> | 推到 `main` → Vercel 出一次生产部署，域名在项目域名表里，自动跟上 |
| 备 | <https://mrbaoboer.github.io/3D-Bike-Builder/> | `.github/workflows/deploy.yml` → GitHub Pages |

两处都是全自动的，发版之后不需要任何手工步骤。`npm run live` 对一下两处发的是不是
同一份产物 —— 判据是入口 JS 的内容哈希，只看「打得开」是不够的。
