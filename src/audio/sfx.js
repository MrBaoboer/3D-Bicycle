 ...
 * 金属味来自衰减比 —— 钢的内耗极低，各阶模态衰减时间差不多，泛音陪着基频响完；
 * 改配方只动频率不动衰减比，钢件立刻变成敲木头。
...
const RECIPES = {
   ...
   *  · 之后    杆身弯曲模态 1 : 2.76 : 5.4 : 8.9，衰减时间几乎一样平（金属味来源）
   *  · 190 Hz  手握着工具，低频被掌心吃掉大半，只留一记短促的推力
   *  · +14 ms  一记弱一个数量级的回响，不盖过正音
   */
  SNUG_CLICK: (S, t, p, g) => {
   ...
   * 先是贴合面的滑动摩擦，中心 1.5 kHz 往下走（接触面越贴越多，高频先没）；
   * 末端质量被挡住 —— 100→50 Hz 低频冲击加车架回应的 340 Hz 中频模态，
   * 再叠一层管腔排气的低通气流。
   ...
   * 敲上去发闷。两个相差约 4.5 Hz 的模态叠成一记慢拍，听感是「没对上」；
   * 起音留到 12 ms 不刺人；音高从头到尾不动 —— 一下行就成了游戏机的失败提示。
   */
  WRONG: (S, t, p, g) => {
    S.modes(t, {
