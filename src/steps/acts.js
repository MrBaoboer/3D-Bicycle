 ...
 *
 * 每一步是一份声明加两个钩子，形状见 docs/DEVELOPMENT.md。
...
export function acts(ctx) {
  ...
  return [
    ...
    P({
       ...
       * 避震是**一整根**，不是半根。
       *
       * 模型按悬挂动画的需要把它拆在了两个父级下：主体与下眼挂在摇臂的连杆上，
       * 上眼那一段挂在主车架上。清单原先只认头一个节点，于是上眼那一截一直算车架的 ——
       * 「从一根车架开始」那张光车架照上挂着一截来路不明的黑筒，
       * 到了这一步又凭空掉下来半根避震。四个节点现在归一件，见清单里 shock 的 note。
       */
      id: 'B3',
      phase: 1,
      title: '后避震',
      ...
      note: {
        ...
        body: '避震两端各有一个眼孔：上眼对进主车架的避震座，下眼对进摇臂那根连杆，'
          ...
          + '眼孔里是衬套，所以拧死之后两头仍然能摆：后轮每动一次，避震就绕着这两颗螺栓转一点。'
          + '<em>这份模型里没有这两颗螺栓，所以这一步只演到落进两个座为止。</em>'
    ...
    P({
      ...
      parts: ['handlebar'],
      /*
       * 这一步站到车的正侧方，不用 viewFor 报的那个前四分之三位。
       *
       * 车把是**顺着车身长轴**往后坐进托座的（装配方向 [0.906, −0.423, 0]），
       * 而那条轴在正侧方的画面里是横着摊开的一整条 —— 「往后推」这件事
       * 在屏幕上就是一段实实在在的横移。前四分之三位上它大半落在纵深里，
       * 看着更像车把在慢慢变大。侧方还顺带把托座、舵管、把立三者的关系摆平了。
       *
       * 停在中线略左、略高：车头朝画面左，车把落在偏左那一档，右边留给它要坐进去的车身，
       * 眼睛顺着行程方向往前看得见落点。略高是因为这一刻车上的东西全在车把下方 ——
       * 主体压在正中时，上半幅整个是空的，而底下的前叉又被切在画幅外。
       *
       * 正侧方（az 90）试过，不用：车把是横在车身上的，正对着它看就成了端着看，
       * 屏幕上只剩 9% 宽，而它 0.78 m 的纵深还要把镜头顶到 1.25 m 外 ——
       * 一根认不出是车把的棕色短桩。105° 已经是侧方那一档（车身侧影完整），
       * 又还留得下把宽的三分之一。
       */
      cam: { az: 105, el: 14 },
      off: [-0.13, 0.14],
