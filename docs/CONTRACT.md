...
**主体默认落在舞台正中**，锚点取进场那一刻的预备位。要偏就在这一步写
`off: [右为正, 上为正]`（以取景半跨度计，车把那一步是 `[-0.13, 0.14]`）——
不写就是正中，偏出去要有人签字。`npm run frames` 的「主体 x,y」那一栏盯着这件事，
超过 0.30 当场退 1。
...
- 屏幕上看不出来的位移（`TINY`）直接落位，一帧都不动画。这不是跳切 ——
  按定义它在屏幕上就不可见。
- 中段外扩只留给真的要绕过去的那几趟（`BULGE_FROM` 40°）。小幅调整时它只是一次多余的呼吸。
  实测二十八趟里只有六趟有外扩。
