...
**线上地址必须是项目域名表里的那一条，不要用手工别名。**

`build-bike.vercel.app` 在项目的域名表里，每次生产部署自动指过去 ——
不需要任何人管，也不需要任何工作流。

这里踩过一个坑，值得留一句。曾经拿 `3d-bicycle.vercel.app` 当主地址，
它是**部署级别名**而不是项目域名 —— 别名指的是某一个具体部署，不跟着生产走，
于是每发一版就掉队一次，而且照样返回 200、标题一样，从外面看不出来。
想把它补进域名表也补不进去：Vercel 的 `vercel.app` 前缀全局先到先得，
某个前缀一旦在**项目域名层级**被占用就不放回公共池，support 也无权清理
（2026-08-16 官方答复）。一度靠一条工作流发一次指一次顶着，
后来整个放弃这个名字，换成没被占的 `build-bike.vercel.app`，那条工作流随之删掉。

判断办法很简单 —— `vercel domains add <名字>.vercel.app <项目>` 能成，
它就是项目域名、会自动跟随；报 `alias_conflict` 就换个名字，别用别名硬顶。
